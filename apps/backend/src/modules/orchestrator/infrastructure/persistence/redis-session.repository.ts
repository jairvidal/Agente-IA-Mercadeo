...
import { MAX_HISTORY_TURNS, SESSION_TTL_SECONDS } from "@/modules/orchestrator/domain/constants";
...
interface SerializedSession {
  ...
  channel: ChannelType;
  userId: string;
  history: Array<{ role: ConversationTurn["role"]; content: string; timestamp: string }>;
...
export class RedisSessionRepository implements SessionRepositoryPort {
  ...
  async getOrCreate(userId: string, channel: ChannelType): Promise<Session> {
