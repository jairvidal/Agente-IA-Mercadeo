...
class FakeMcpClient implements McpClient {
  ...
  readResourceImpl: (uri: string) => Promise<string> = async () => "default-catalog";
  getPromptImpl: (name: string, args: Record<string, string>) => Promise<string> = async () =>
    "default-prompt";
...
describe("McpKnowledgeBaseAdapter", () => {
  ...
  describe("getFaqCatalog", () => {
    it("reads the faq://catalog resource the first time", async () => {
      ...
      expect(result).toBe("FAQ CATALOG TEXT");
    ...
    it("wraps MCP failures in KnowledgeBaseUnavailableError", async () => {
      mcp.readResourceImpl = async () => {
        throw new McpTimeoutError(10000);
      };

      const promise = kb.getFaqCatalog();
      await expect(promise).rejects.toBeInstanceOf(KnowledgeBaseUnavailableError);
    });
  });
  ...
  describe("getSystemPrompt", () => {
    ...
    it("wraps MCP failures in KnowledgeBaseUnavailableError", async () => {
      ...
      const promise = kb.getSystemPrompt({ context: "ctx" });
      await expect(promise).rejects.toBeInstanceOf(KnowledgeBaseUnavailableError);
    });
  });
});
