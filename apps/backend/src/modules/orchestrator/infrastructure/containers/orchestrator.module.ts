...
import { buildLlm, type OrchestratorConfig } from "../config/orchestrator.config";
...
export async function buildOrchestratorModule(
  ...
  const habeasDataConsent = new HandleHabeasDataConsentUseCase({ sessions });

  const processMessageUseCase = new ProcessTelegramMessageUseCase({
    channel,
    sessions,
    classifier,
    registry,
    habeasDataConsent,
    clock: () => new Date(),
    businessHoursGateEnabled: config.businessHoursGateEnabled,
  });
