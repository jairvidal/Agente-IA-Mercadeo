...
export class ProcessTelegramMessageUseCase {
  ...
  async execute(
    ...
    //
    // Runs BEFORE we append the user turn so the handled=true branch owns
    // turn persistence end-to-end (user + assistant) and can short-circuit
    // without us double-appending the user message. The gate fires for any
    // session without resolved consent — brand-new or legacy — so existing
    // users get prompted on their next message after this rolls out.
    // When the gate handles the message, we never reach the classifier.
    const consentResult = await this.consent.execute(
      sessionResult.value,
      message.text,
    );
    ...
    const withUserResult = await this.sessions.appendTurn(
      sessionForClassifier,
      {
        role: "user",
        content: message.text,
      },
    ...
    switch (args.category) {
      ...
      case "commercial_quotation": {
        ...
        if (!agent) {
          ...
          logger.error(
            `No agent resolved for commercial category ${args.category}`,
          );
          return {
            text: AGENT_FALLBACK_REPLY,
  ...
  private intentForAgentError(error: {
    name: string;
    message: string;
  }): string {
    switch (error.name) {
