...
/**
 * Specialized agent for informational ("commercial_faq") turns.
 *
 * Responsibilities:
 *   1. Fetch the FAQ catalog from the KB.
 *   2. Fetch the FAQ-specific system prompt from the KB.
 *   3. Delegate to `BaseAgent.executeLoop`.
 */
export class FaqAgent extends BaseAgent implements AgentPort {
	readonly name: AgentName = "faq";
	protected readonly allowedTools: ReadonlySet<string>;
	private readonly knowledgeBase: KnowledgeBasePort;

	constructor(deps: FaqAgentDeps) {
		super({
			llm: deps.llm,
